...
String getData() {
  ...
  String getDataString = "{type:'getData',qiwen:'" + String(qiwenM.value) + "',guangzhao:'" + String(guangzhaoM.value) + "',shidu:'"
                         ...
                         + "',weishiState:'" + String(weishiM.state) + "'}";
...
void setup() {
  ...
  if (!MDNS.begin("aigs")) { //http://aigs.local
  ...
  display = Adafruit_SSD1306(SCREEN_WIDTH, SCREEN_HEIGHT, &Wire, -1); //初始化OLED
  ...
  display.setTextColor(WHITE);//设置OLED字体颜色
